...
describe('#order-use-case', () => {
  ...
  describe('#ensureFunds', () => {
    ...
    it('should throw an error if the wallet doesnt have enough tokens', async () => {
      try {
        ...
        assert.include(error.message, 'App wallet does not have enough tokens to satisfy the SELL order')
      }
    })

    it('should return true if buy', async () => {
      const orderEntity = { buyOrSell: 'buy' }
      const result = await uut.ensureFunds(orderEntity)
  ...
  describe('#moveTokens', () => {
    it('should move tokens to the holding address', async () => {
      ...
      const result = await uut.moveTokens(orderEntity, walletInfo)
    ...
    it('should catch an error', async () => {
      ...
      try {
        const walletInfo = await uut.getAddress(1)
        await uut.moveTokens(orderEntity, walletInfo)
        assert.fail('unexpected result')
      } catch (error) {
  ...
  describe('#findOrderByHash', () => {
    it('should return an order by a given p2wdb hash', async () => {
      sandbox.stub(uut.OrderModel, 'findOne').resolves({
        toObject: () => ({
          _id: '61d8c3b6faabfd0d5f18cae7',
          messageType: 1,
          messageClass: 1,
      ...
      const order = await uut.findOrderByHash('zdpuAowSDiCFRffMBv4bv4zsNHzVpStqDKZU4UBKpiyEsVoHE')
    ...
    it('should throw an error if the given hash doesnt match any order', async () => {
      const consoleSpy = sandbox.spy(global.console, 'error')
